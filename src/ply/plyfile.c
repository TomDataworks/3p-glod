    ...
    )
{
    ...
    if (filename != NULL)
    {
        ...
        fp = fopen (name, "wb"); // b prevents EOL translation on some systems
        if (fp == NULL) {
            return (NULL);
        }
    }
    ...
    )
{
    ...
    if (filename != NULL)
    {
        ...
        fp = fopen (name, "rb"); // b prevents EOL translation on some systems
        if (fp == NULL)
            return (NULL);
    }
    else
...
void ply_close(PlyFile *plyfile)
{
    ...
    fclose (plyfile->fp);
    for (i = 0; i < plyfile->nelems; i++) {
        ply_free_element(plyfile->elems[i]);
        free(plyfile->elems[i]);
    }
    free(plyfile->elems);
